...
VAMBAIR = Token:match("(%d+)")
redis:set(VAMBAIR..":VERSION",1)
redis:set(VAMBAIR..":SUDO_ID:",SUDO_USER)
redis:set(VAMBAIR..":DataCenter:",'German')
redis:set(VAMBAIR..":UserNameBot:",BOT_User)
redis:set(VAMBAIR..":NameBot:",BOT_NAME)
redis:hset(VAMBAIR..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3VAMBAIR_INSTALL","Yes")
info = {}
...
VAMBAIR = Token:match("(%d+)")
our_id = tonumber(VAMBAIR)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(VAMBAIR..":UserNameBot:")
SUDO_ID = tonumber(redis:get(VAMBAIR..":SUDO_ID:"))
SUDO_USER = redis:hgetall(VAMBAIR..'username:'..SUDO_ID).username
version = redis:get(VAMBAIR..":VERSION")
DataCenter = redis:get(VAMBAIR..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..VAMBAIR..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(VAMBAIR..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(VAMBAIR..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(VAMBAIR..'group:add'..msg.chat_id_) then 
...
msg.TheRank =  '𝐃𝐄𝐕 𝐕𝐀𝐌𝐁𝐄𝐑'
msg.Rank = 1

elseif msg.sender_user_id_ == 1458654225 then 
msg.TheRank =  '𝐃𝐄𝐕 𝐁𝐎𝐘𝐊𝐀🇪🇬.'
msg.Rank = 1
...
msg.Rank = 1
elseif redis:sismember(VAMBAIR..':THANWY_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'مطور اساسي²'
msg.TheRank = 'مطور اساسي²'
msg.Rank = 1
elseif redis:sismember(VAMBAIR..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(VAMBAIR..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(VAMBAIR..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(VAMBAIR..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(VAMBAIR..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(VAMBAIR..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(VAMBAIR..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(VAMBAIR..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(VAMBAIR..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(VAMBAIR..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(VAMBAIR..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(VAMBAIR..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس فامبير ⇐ @Xx_SorceVambair_xX
...
if msg.GroupActive and redis:get(VAMBAIR..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(VAMBAIR..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(VAMBAIR..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(VAMBAIR..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.VAMBAIR then 
if msg.text and ScriptFile.iVAMBAIR then
for k, VAMBAIR in pairs(ScriptFile.VAMBAIR) do
local SearchText = msg.text:match(VAMBAIR)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iVAMBAIR(msg,{msg.text:match(VAMBAIR)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",VAMBAIR.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dVAMBAIR then
if ScriptFile.dVAMBAIR(msg) == false then
return false
end
...
if Plug.VAMBAIR then 
if msg.text and Plug.iVAMBAIR then
for k, VAMBAIR in pairs(Plug.VAMBAIR) do
local SearchText = msg.text:match(VAMBAIR)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(VAMBAIR..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(VAMBAIR..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iVAMBAIR(msg,{msg.text:match(VAMBAIR)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",VAMBAIR.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dVAMBAIR then
Plug.dVAMBAIR(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source VAMBAIR")
end 

end
else
print("The File Script.lua Not Runing in The Source VAMBAIR")
...
function tdcli_update_callback(data)
	...
	redis:srem(VAMBAIR..'users',msg.sender_user_id_)
	end
	...
	EditMsg(data.message_.chat_id_,data.message_.id_,'100% - |SOURCE VAMBAIR|\n\n• ** السورس الى اصدار \n• ** تم اعاده تشغيل السورس بنجاح')
	dofile("./inc/Run.lua")
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(VAMBAIR..":SUDO_ID:")
redis:hset(VAMBAIR..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(VAMBAIR..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(VAMBAIR..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(VAMBAIR..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(VAMBAIR..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(VAMBAIR..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(VAMBAIR..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(VAMBAIR..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(VAMBAIR..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(VAMBAIR..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(VAMBAIR..'group:add'..data.chat_.id_) then
	redis:set(VAMBAIR..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(VAMBAIR..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(VAMBAIR..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(VAMBAIR..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	VAMBAIR..'group:name'..k,IDS.Title,
	VAMBAIR..'num_msg_VAMBAIR'..k,5,
	VAMBAIR..'group:add'..k,true,
	VAMBAIR..'lock_link'..k,true,
	VAMBAIR..'lock_spam'..k,true,
	VAMBAIR..'lock_webpage'..k,true,
	VAMBAIR..'lock_markdown'..k,true,
	VAMBAIR..'lock_flood'..k,true,
	VAMBAIR..'lock_bots'..k,true,
	VAMBAIR..'mute_forward'..k,true,
	VAMBAIR..'mute_contact'..k,true,
	VAMBAIR..'mute_document'..k,true,
	VAMBAIR..'mute_inline'..k,true,
	VAMBAIR..'lock_username'..k,true,
	VAMBAIR..'replay'..k,true
	)
	redis:sadd(VAMBAIR..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(VAMBAIR..'username:'..ID,'username',user)
	redis:sadd(VAMBAIR..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(VAMBAIR..'username:'..ID,'username',user)
	redis:sadd(VAMBAIR..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(VAMBAIR..'username:'..ID,'username',user)
	redis:sadd(VAMBAIR..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(VAMBAIR..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(VAMBAIR..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(VAMBAIR..'username:'..data.user_.id_).username
	...
	redis:hset(VAMBAIR..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
